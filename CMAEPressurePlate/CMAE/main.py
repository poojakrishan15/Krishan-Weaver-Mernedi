...
def eval(policy1, policy2, policy3, policy4, eval_env, raw_obs_dim):
    print('EVAL')
    episode_rew = 0
    s = eval_env.reset()
    episode_rews = []
    for _ in range(10):
        while True:
            s, _ = obs_to_int_pi(s, base=15, raw_dim=raw_obs_dim)
            #updated from 2 to 4 agents
            a1 = policy1.select_action(s, 0)
            a2 = policy2.select_action(s, 0)
            a3 = policy3.select_action(s, 0)
            a4 = policy4.select_action(s, 0)
            s_next, r, done, _ = eval_env.step([a1, a2, a3, a4])
            #added the mean reward to handle each agent getting a different rerward in pressure plate
            episode_rew += mean(r)
            s = s_next
            if done:
...
def main():
    ...
    #added two more count tables and adjusted the size since the pressure plate environment is not a square grid
    agent1_count = np.zeros((15, 9), dtype=np.int32)
    agent2_count = np.zeros((15, 9), dtype=np.int32)
    agent3_count = np.zeros((15, 9), dtype=np.int32)
    agent4_count = np.zeros((15, 9), dtype=np.int32)
    ...
    s_raw = env.reset()
    s, s_p = obs_to_int_pi(s_raw, base=15, raw_dim=raw_obs_dim)
    episode_rew = 0
    ...
    while True:
        env.render()
        #time.sleep(0.0001)
        #altered how to pass in the s_raw values to match the obs style of the pressure plate environment
        agent1_count[s_raw[0][1], s_raw[0][0]] += 1
        agent2_count[s_raw[1][1], s_raw[1][0]] += 1
        agent3_count[s_raw[2][1], s_raw[2][0]] += 1
        agent4_count[s_raw[3][1], s_raw[3][0]] += 1
        episode_step += 1
        total_step_count += 1
        ...
        if args.mixed_explore:
            ...
            a1 = q_learner1.select_action(s[0], eps, q_learner_target1.q_table, alpha)
            a2 = q_learner2.select_action(s[0], eps, q_learner_target2.q_table, alpha)
            a3 = q_learner3.select_action(s[0], eps, q_learner_target3.q_table, alpha)
            a4 = q_learner4.select_action(s[0], eps, q_learner_target4.q_table, alpha)
        else:
        ...
        #This line was unclear on how to adapt to 4 agents, assumed to add a3 and a4 since a2 was added in the formula
        a = env.action_space.nvec[0] * a1 + a2 + a3 + a4
        if meta_counter.count[s[0], a] == 0:
            n_new_state_action += 1
        meta_counter.update_count(s[0], a)
        if 'active' in args.exp_mode:
            meta_q.update_count(s[0], a, multilevel=args.multilevel,
                                all_subspace=args.all_subspace,
                                tree_subspace=args.tree_subspace)
        episode_actions.append([a1, a2, a3, a4])
        episode_states.append(s[0])
        if 'active' in args.exp_mode and episode_count >= 1 and (
                s[0] == goal_s or s[0] == s_to_sp(goal_s, base=15, raw_dim=raw_obs_dim)):
            steps_to_goal = episode_step
        s_next_raw, r, done, _ = env.step([a1, a2, a3, a4])
        s_next, s_next_p = obs_to_int_pi(s_next_raw, base=15, raw_dim=raw_obs_dim)
        rAvg = mean(r)
        #Altered how data was inserted to the buffer to match pressure plate obseravations
        if 'active' in args.exp_mode:
            q_learner1.insert_data(s[0], a1, r[0], s_next[0], done[0])
            q_learner2.insert_data(s[0], a2, r[1], s_next[0], done[1])
            q_learner3.insert_data(s[0], a3, r[2], s_next[0], done[2])
            q_learner4.insert_data(s[0], a4, r[3], s_next[0], done[3])
            q_learner_target1.insert_data(s[0], a1, r[0], s_next[0], done[0])
            q_learner_target2.insert_data(s[0], a2, r[1], s_next[0], done[1])
            q_learner_target3.insert_data(s[0], a3, r[2], s_next[0], done[2])
            q_learner_target4.insert_data(s[0], a4, r[3], s_next[0], done[3])
            meta_q.insert_data(s[0], a, rAvg, s_next[0], done[3])
        else:
        ...
        s = s_next
        s_raw = s_next_raw
        #changed to check if the last agent has reached its goal state or if H steps have been observed
        if done[3]:
            ...
            if 'active' in args.exp_mode:
                 # The remaining section is where our adaptation of CMAE breaks, unsure how to adapt meta_q.get_goal and update_q_from_D
                goal_s, goal_a = meta_q.get_goal(multilevel=args.multilevel, all_subspace=args.all_subspace)
                #This was another unclear line on how to adapt to 4 agents, the line below is in the orginal algorithm
                #Seems to heavily assume only 2 agents are present, decided that the best approach was to continue giving the 
                #same values to agents 3 and 4
                goal_a1, goal_a2 = divmod(goal_a, n_actions_per_agent)
                goal_a3, goal_a4  = divmod(goal_a, n_actions_per_agent)
                if episode_count % 20 == 0:
                ...
                q_learner_target3.update_q_from_D(reset_q=False)
                q_learner_target4.update_q_from_D(reset_q=False)
                q_learner1.update_q_from_D(goal=(goal_s, goal_a1))
                q_learner2.update_q_from_D(goal=(goal_s, goal_a2))
                q_learner3.update_q_from_D(goal=(goal_s, goal_a3))
                q_learner4.update_q_from_D(goal=(goal_s, goal_a4))
                meta_q.goal_q.clear()
            episode_actions = []
            episode_states = []
            ...
            if episode_count % args.eval_every_episode == 1:
                ...
                if 'active' in args.exp_mode:
                    policy1, policy2, policy3, policy4 = q_learner_target1, q_learner_target2, q_learner_target3, q_learner_target4
                else:
                    policy1, policy2 = q_learner1, q_learner2
                eval_rew = eval(policy1, policy2, policy3, policy4, eval_env, raw_obs_dim)
