...
class PressurePlate(gym.Env):
    ...
    def __init__(self, H, height, width, n_agents, sensor_range, layout):
        ...
        self.step_count = 0
        #convert spaces.Tuple to spaces.MultiDiscrete
        self.action_space = spaces.MultiDiscrete(n_agents * [5])
        self.action_space_dim = (sensor_range + 1) * (sensor_range + 1) * 4 + 2
        #Updated observation space to match the other environments where each entriy is the size of one side of the grid world
    ...
    def step(self, actions):
        ...
        #keep track of steps in an episode so that it works with the H value in CMAE
        self.step_count += 1 
        ...
        #added the check for the step limit aka H
    ...
    def _get_obs(self):
        ...
        for agent in self.agents:
            ...
            #Altered the obs to only transfer the location of each agent and not the padding from above
            obs.append(np.array([x, y]))
        #also add to the observation if each pressure plate is pressed to match the other environments
        plateList = list()   
        for i in range(3):
            plate_loc = [self.plates[i].x, self.plates[i].y]
            if plate_loc[0] == obs[i][0] and plate_loc[1] == obs[i][1]:
                plateList.append(1)
            else:
                plateList.append(0)
        plateState = plateList[0] * 2**2 + plateList[1] *2 + plateList[2]
        obs.append(plateState)
