    ...
    def get_goal_tractories(self, goal: (int, int), goal_replay_buffer: 'Storage', base: int):
        ...
        for ind in goal_s_inds:
            ...
            goal_replay_buffer.batch_insert(self.state[ind - t: ind], self.action[ind - t: ind],
                                            ...
                                            self.done[ind - t: ind])
            goal_replay_buffer.insert(goal_s, goal_a[0], 1, goal_s, 1)
        return True
...
class ActiveQLearning(QLearning):
    ...
    def get_goal_proj(self, level, count_id, bz=1024, all_subspace=False):
        ...
        #check if count table is being updated while training
        print(count)
        #This is the helper function that is currently breaking, broke up the the line below into two
        #because "np.where(count == np.min(count))" was only producing one list of values
        #could be the potential source of the current error
        #goal_s_ids, goal_as = np.where(count == np.min(count))
        goal_s_ids = np.where(count == np.min(count))
        goal_as = np.where(count == np.min(count))
        goal_ss = states[goal_s_ids]
        goal_idx = np.random.randint(0, len(goal_as))
