...
def main():
    ...
    # print([a.shape[0] for a in env.observation_space])
    raw_obs_dim = [a.shape[0] for a in env.observation_space]
    agent1_count = np.zeros((env.grid_size[0], env.grid_size[1]), dtype=np.int32)
    agent2_count = np.zeros((env.grid_size[0], env.grid_size[1]), dtype=np.int32)
    n_states = reduce(lambda x, y: x * y, [a.shape[0] for a in env.observation_space])
    kargs = {'n_states': n_states,
             'n_actions': reduce(lambda x, y: x * y, [a.shape[0] for a in env.action_space]),
             'base': env.grid_size,
             'raw_dim': len(env.observation_space),
    ...
    if args.exp_mode == 'active_cen':
        ...
        kargs['n_actions'] = [a.shape[0] for a in env.action_space][0]
    ...
    s_raw = env.reset()
    s = obs_to_int_pi(s_raw, base=3, raw_dim=raw_obs_dim)
    print(s_raw,s)
    if s!=None:
        s=s,s_p
    episode_rew = 0
    ...
    n_actions_per_agent = [a.shape[0] for a in env.action_space][0]
    ...
    while True:
        print("\n")
        print(s_raw[1][1],s_raw[0][0])
        # print(s_raw[3],s_raw[2][0])
        agent1_count[int(s_raw[1][1]), int(s_raw[0][0])] += 1
        agent2_count[int(s_raw[1][3]), int(s_raw[1][2])] += 1
        episode_step += 1
        total_step_count += 1
        ...
        a = [a.shape[0] for a in env.action_space][0] * a1 + a2
        if (meta_counter.count[s, -1] == 0).all():
