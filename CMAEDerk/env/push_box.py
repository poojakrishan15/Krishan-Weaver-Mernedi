...
class PushBox(object):
    ...
    def render(self):
        ...
        grid = [[0 for x in range(self.grid_size)] for y in range(self.grid_size)]
        ...
        for row in range(self.grid_size):
            for column in range(self.grid_size):
                color = WHITE
                if self.agents[0].y == row and self.agents[0].x == column:
                    color = BLUE
                if self.agents[1].y == row and self.agents[1].x == column:
                    color = RED 
                if self.box.y == row and self.box.x == column:
                    color = GREEN
                pygame.draw.rect(screen,color,[(MARGIN + WIDTH) * column + MARGIN,(MARGIN + HEIGHT) * row + MARGIN,WIDTH,HEIGHT])
        # for i in self.agents:
        #     print("Agents",i.x,i.y)

        # print("Box",self.box.x,self.box.y)
        clock.tick(60)
        pygame.display.flip()
